...
class ImageCropper(object):
	def crop_and_create_images(self, card_id: int) -> DeploymentResult:
		...
		home = expanduser("~")
		if any(not path.exists( join_paths(home, crop.original_path)) for crop in crops):
			result.deployed = False
			result.status = "Could not find source file: {}".format(next(crop for crop in crops if not path.exists(crop.original_path)).original_path)
			return result

		for crop in crops:
			nl = self.convert_to_csv_line(crop)
			print("Creating crop for {}".format(path.basename(crop.target_path)))
	...
	def convert_to_csv_line(self, crop_ins: CardCropInstruction):
		...
		return "{},{},{},{},{},{},{},{}".format(original_path, target_path, crop_ins.target_width,
												...
												crop_ins.crop_start_x_pc, crop_ins.crop_start_y_pc,
												crop_ins.crop_end_x_pc,
	...
	def deploy_sound_for_card(card_id: int):
		...
		if sound_src_path is None:
			...
			if not os.path.exists(sound_src_path):
				result.deployed = False
				result.status = "Source sound for card id {0} not found: {1}".format(card_id, sound_src_path)
			else:
