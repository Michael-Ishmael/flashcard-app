...
public struct ImageFormatDef : Decodable
{
    ...
    public init?(json: JSON) {
        ...
        guard let landscape:ImgSrc = "landscape" <~~ json,
            let portrait:ImgSrc = "portrait" <~~ json,
            let imageType:ImageType = "imagetype" <~~ json
            else {
                return nil
        }
...
public struct FlashCard : Decodable
{
    ...
    public init?(json: JSON) {
        guard let id:Int = "id" <~~ json,
            let index:Int = "index" <~~ json,
            let sound:String = "sound" <~~ json,
            let imageDef:[String: ImageFormatDef] = "imagedef" <~~ json
            else {
                return nil
        }
...
public struct FlashCardDeck : Decodable {
    ...
    public init?(json: JSON) {
        guard let id:Int = "id" <~~ json,
            ...
            let cards:[FlashCard] = "cards" <~~ json
            else {
                return nil
        }
...
public struct FlashCardSet : Decodable {
    ...
    public init?(json: JSON) {
        guard let id:Int = "id" <~~ json,
            let name:String = "name" <~~ json,
            let icon:String = "icon" <~~ json,
            let decks:[FlashCardDeck] = "decks" <~~ json
            else {
                return nil
        }
