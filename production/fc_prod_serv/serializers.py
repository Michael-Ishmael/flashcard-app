...
from fc_prod_serv.models import MediaFile, MediaFileType, Config, Set, Deck, Card, Crop, AspectRatio, Orientation, \
	TargetDevice, CardCropInstruction, CardTargetDevice, CroppingInstruction, DeckAssignmentStatus
...
class DeckSerializer(serializers.HyperlinkedModelSerializer):
	...
	def get_incomplete_count(self, deck: Deck):
		related_status = DeckAssignmentStatus.objects.get(deck_id=deck.deck_id)
		if related_status is not None:
			return related_status.incomplete
		else:
			return -1

	def get_complete_count(self, deck: Deck):
		related_status = DeckAssignmentStatus.objects.get(deck_id=deck.deck_id)
		if related_status is not None:
			return related_status.complete
		else:
			return -1

	class Meta:
		model = Deck
		fields = ('deck_id', 'name', 'set_id', 'icon', 'display_order', 'speech', 'incomplete_count', 'complete_count')
...
class CardSerializer(serializers.HyperlinkedModelSerializer):
	...
	speech = serializers.SlugRelatedField(many=False,
										  queryset=MediaFile.objects.filter(media_file_type=4),
										  slug_field="relative_path", required=False, allow_null=True)
